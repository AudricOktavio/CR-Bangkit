...
@app.get("/v1/api/gateway/user/virtualmachines", response_model=list[schemas.Vm])
def read_vms(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_vms(db, skip=skip, limit=limit)
    return users

@app.get("/v1/api/gateway/user/virtualmachines/{vm_id}", response_model=schemas.Vm)
...
async def create_code(id: int, action: str, db: Session = Depends(get_db)):
    ...
    #send notification
    notirec = db.query(User.token).join(Ownership).filter(Ownership.vm_id == id).all()
...
def stvm(vm_id: int, request:str, vcode:str, release_ip: bool = False, db: Session = Depends(get_db)):
...
def stvmp(vm_id: int, request:str, vcode:str, release_ip: bool = False, db: Session = Depends(get_db)):
    vmc = db.query(models.Vm).filter(models.Vm.id == vm_id).first()
    if request == "start" and vmc.is_active == True:
        crud.start_vmp(db, vm_id=vm_id, vcode=vcode)
    elif request == "stop" and vmc.is_active == True:
        crud.stop_vmp(db, vm_id=vm_id, vcode=vcode)
    elif request == "destroy" and vmc.is_active == True:
        crud.delete_vmp(db, vm_id=vm_id, vcode=vcode)
    else:
        return "there is no such request"
    return "success"